#<------------------ MACRO DEFINITIONS ---------------------->#
	...
	# Macro: lwi
	# Usage: loads 	two 16-bit immediate values in higher and lower bits in $reg
	.macro lwi($reg, $ui, $li)
	lui $reg, $ui		# Puts $ui in higher bits
	addi $reg, $reg, $li	# Adds $li to $ui in lower bits
	.end_macro
	
	# Macro: push
	# Usage: pushes data into stack and increases $sp
	.macro push($reg)
	sw $reg, 0($sp)		# Stores $reg at $sp
	addi $sp, $sp, -4	# Increments $sp by 4
	.end_macro
	
	# Macro: pop
	# Usage: removes data from stack and decreases $sp
	.macro pop($reg)
	addi $sp, $sp, 4	# Decrements $sp by 4
	lw $reg, 0($sp)		# Loads $sp to $reg
	.end_macro
